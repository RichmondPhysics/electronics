...
\textbf{Getting Acquainted With Your Diodes}

\begin{enumerate}[wide]

\item The circuit diagram below  shows two resistors connected in series to a transformer.  Although the diagram shows the same transformer you used in Lab 10, you should instead use the 12.6~VAC transformer on your PB-505.  The diagram also shows your oscilloscope measuring the voltages across both of the resistors, but instead of graphing them both vs. time, here you will graph the voltage across one resistor vs. the voltage across the other.  To do this, under the ``acquire'' menu of your scope, select ``XY format ON.''   Also, under the channel 2 menu, select ``invert on.''  Since the two resistors in your circuit are identical, the display here should show a 45 degree diagonal line, the graph of $y = x$.  What does your display show if you disconnect either the channel 1 or channel 2 probe?
\vspace{-0.1in}
\begin{center}
...
\item If you replace the first resistor in your circuit with a diode, your oscilloscope shows you a graph of current vs. voltage for the diode.  Channel 1 (The X coordinate) is the voltage across the diode, and Channel 2 (The Y coordinate) is the voltage across the resistor, which is proportional to the current.  Use this to measure the $IV$ curve of the following diodes: 1N4001, 1N4733, 1N4739, 1N34 (which may have no label, but is identifiable by two black lines), as well as a red LED and a blue LED (which may appear clear).  What are their forward and reverse breakdown voltages, $V_F$ and  $V_R$?  Each of the four diodes that isn't an LEDs may be either: a regular silicon diode, a germanium diode, or a Zener diode.  Identify the type of each one.  (Silicon, Germanium, Zener....) \label{part_measuring_diode_iv} \label{part_diode_measurements}
\vspace{-0.1in}
\begin{center}
...
\textbf{Series Clippers and Shunt Clippers}

One of the things that diodes are often used for is ``clipping'' a signal; that is, making it so that a voltage never goes above a predetermined limit or never goes below a predetermined limit.  Clippers can be applied to audio signals to produce a distorted sound (this is the basis for distortion pedals used for electric guitars), and clippers can also be used to protect other parts of a circuit from overvoltages or undervoltages.

\item On your scope, turn XY format mode OFF, and also \textit{do not forget to turn the Ch2 invert OFF}.  Build the ``series clipper'' circuit%
\footnote{Honestly, it seems a little pretentious to call it a ``circuit'' since it's really just one lousy diode.}
pictured below, using a 1N4001 diode and a load resistance of 100~k$\Omega$.  Make a sketch of the input waveform and the output waveform.  Make precise measurements of the peak positive and negative input voltages, and the peak positive and negative output voltages.  Are they \textit{exactly} the same?  (You may want to shift the vertical positions of your traces and increase the scale on your scopes for more precise measurements.)  Also predict how your graph will change if you reverse the polarity of the diode, and test your prediction. \label{part_series_clipper}
\begin{center}
...
%\pagebreak
\item The circuit below is called the ``biased shunt clipper,'' because the diode is now connected to a non-zero reference voltage (the ``bias'' voltage) instead of to ground.  Build the circuit and sketch the resulting output waveform for $V_{\rm out}$.  What is the relationship between $V_{\rm out}$ and the voltage at the junction between $R_1$ and $R_2$?  
...
\item Why is it necessary to use an op-amp in the biased shunt clipper above, rather than connecting the diode to $R_1$ and $R_2$ directly?  If you did omit the op-amp, how would you have to adjust the values of your resistors? (No need to build this.)
