...
\hfill\includegraphics{M_problems/the_basics/real_battery_10ohms.pdf}
\end{minipage}
...
\smallskip
\hfill\includegraphics{M_problems/the_basics/voltage_divider.pdf}
\end{minipage}
...
(b)~For a supply with a fixed, unchangable $R_{\rm int}$, what value should you choose for $R_{\rm load}$ to \textit{maximize} the power dissipated in the load?
(c)~For a fixed, unchangeable $R_{\rm load}$ what value would you want to choose for $R_{\rm int}$ to \textit{maximize} the power  dissipated in the load?
(d)~Based on your answer to part (c), it turns out that some battery types are just better at delivering high load power than others.  Do a little Googling to find how the internal resistance of lithium ion batteries compares to, say, alkaline batteries.  Are any other battery types that stand out as particularly good or particularly bad?
\end{Exercise}
\begin{Answer}
...
%--------------------------------------------------------------------------------------------------------------
\begin{Exercise}
Students often wonder why resistors have the odd values that they often do: for instance, why $2.2\kohm$ and $4.7\kohm$ instead of $2\kohm$ and $5\kohm$? To find out why, check out the Wikipedia entry entitled ``E series of preferred numbers.'' 
\begin{enumerate}[label=(\alph*),nosep]
\item Which set of numbers do the resistors in your kit follow?  (E3? E6? E192?) 
\item In a single sentence, how are the numbers in that specific sequence calculated?
\end{enumerate}
...
\item The voltmeter reads 0 volts.       % b, R2 shorted 
\end{enumerate}
\end{minipage}%
...
\item  $V_{\rm out}$ is a constant 4~V.     % d, Vout connected to pin 1
...
A common problem with trimpots and other potentiometers is that the moveable wiper (pin 2 in the drawing below) doesn't always make good electrical contact with the resistive material between pins 1 and 3.  The resulting \textit{contact resistance} is particularly pernicious because it can fluctuate wildly over time or when the trimpot is adjusted, leading to effects that are both undesireable and unpredictable.
...
(f)~3.0 to 4.62~volts for $R_{\rm contact} =1\kohm$, or constant 3~V for $R_{\rm contact} =\infty$.
