...
\textbf{Inverting and Non-Inverting Amplifiers}

\begin{enumerate}[wide]

\item The circuit below on the left shows a non-inverting amplifier with a gain of 2.  
If $V_{\rm in} = 2$~volts, what are $V_+$, $V_-$, and $V_{\rm out}$?  
Modify this circuit to have a gain of 5, and test it out. (Note: The diagram on the right shows the ``pin-out'' for the LM324 op-amp chip; other pin-out diagrams are in Appendix \ref{appendix_pinouts}.  
Place the LM324 on your breadboard straddling the center line so that you have four additional holes connected to each pin.)
...
\textbf{Input impedance, output impedance, and buffers}

\item Set your function generator to a 500~Hz sine wave at maximum amplitude, and connect it to the speaker at the bottom right of your PB-505.  The speaker has two leads, connected to the left and right sides of the 40-hole breadboard block.  If one speaker lead is connected to the function generator's output, where should the other speaker lead be connected to?  What are the highest and lowest frequencies that you are able to hear?  How does your highest audible frequency compare to the highest audible frequency of your instructor, who may have spent way too much time in college playing in very loud, bad rock bands?

\item Disconnect your function generator from the speaker, and use your friend the oscilloscope to measure both its \textit{open-circuit} voltage%
\footnote{An \textbf{open-circuit} voltage is the output of a source that is not connected to any load---or is connected to a load with a very high input impedance like a voltmeter or an oscilloscope. That is, it's the output voltage in the limit of zero output current.}
and its output voltage when connected to a load resistance of $1\kohm$.  Use these measurements to calculate your function generator's output impedance. \label{part_func_gen_impedance}

\item Set your function generator to a 1~kHz sine wave with an open-circuit RMS voltage of about 100~mV, measured by your oscilloscope.  What happens to that RMS voltage when you connect the function generator to your speaker?  The new RMS voltage will be quite small.  To measure it accurately, you will need to have your probe on 1x, trigger off of your TTL signal, and also use the ``Average'' function on your oscilloscope, which is found within the ``Math'' menu.  What is this ``averaging,'' and how does it help you? 

\item From your measurement above and the known output impedance of your function generator from part~\ref{part_func_gen_impedance}, calculate the input impedance of the speaker.  What is the average power being dissipated in the speaker?  Is it audible? \label{part_nobuffer}

\item The circuit below is a \textit{buffer amplifier} or ``buffer.'' Use the ``golden rules'' of op-amps to determine the voltages $V_+$, $V_-$, and $V_{\rm out}$.  What is the apparent gain of this amplifier?  Can you think of why the heck would you ever want to build such a thing?  (The answer to the last question may be ``no,'' but at least give it a think.)
\begin{center}
\includegraphics{op-amps/buffer.pdf}
\end{center}

\item Build the buffer amplifier (not like there's that much to build, really) and insert the buffer into your circuit for part~\ref{part_nobuffer}; the output of the function generator will be the amplifier's $V_{\rm in}$, and the speaker will be connected to the amplifier's $V_{\rm out}$.  Now what is the RMS voltage across the speaker?  And what is the average power dissipated in the speaker?  What are the apparent input and output impedances of this amplifier?  On a scale of 1 to 10, how cool is this? \label{part_buffer}

\item While monitoring both the $V_{\rm in}$ and $V_{\rm out}$ on your oscilloscope for the circuit in part~\ref{part_buffer}, gradually increase the amplitude of the signal generator.  How do $V_{\rm in}$ and $V_{\rm out}$ change?  What is the maximum current through the speaker?  Is this consistent with the value listed on the data sheet of the LM324? \label{part_clipping}

\bigskip

\textbf{Summing Amplifier and Differential Amplifier}
...
\vspace{-3.7pt}
(for a logical ``1'') or 0 volts (for a logical ``0'').  
These inputs represent the ``ones place,'' the ``twos place,'' the ``fours place,'' and the ``eights place'' of a  number expressed in base two. 
(See Mini-Project~2 on page~\pageref{lab_two_bit_adder} for a short primer on base-two numbers.) 
Show how to use a summing amplifier as part of a 4-bit D/A converter, where the analog output ranges from 0 to +15 volts, corresponding to the numbers 0 through 15.  Note that the output of your summing amplifier will be a negative voltage; how can you make it positive?  (Also: you'll use this circuit again in Mini-Project~1 right after this lab, so don't destroy it when you're done.) 
...
and the output from your 5~V DC supply, and you can change the values of the resistors $R_1$ and $R_2$ to whatever you like.  
\end{minipage}
...
\begin{comment}
\section{First Mini-Project: A Proportional Controller}
\label{prop_control}

 (This part is intended to be long and complicated!)  Your job is to design, build and test a ``proportional controller,'' which could in principle be used to maintain a hotplate at a constant temperature.
\begin{itemize}[nosep]
