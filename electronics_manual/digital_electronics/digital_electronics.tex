...
\item Look carefully at the pinout diagrams for both the 74LS00 (quad NAND) and 74LS02 (quad NOR) in Appendix~\ref{appendix_pinouts}.  Are their inputs and outputs in the same places?  Again, rate your surprise and disappointment with the world on a scale from $-1$ to $-10$.
...
\item \label{part_comparator} Sometimes you want to create a digital signal based on whether one voltage is larger than another.  For example, you may want an alarm to be switched from OFF to ON if some temperature, represented by a voltage, \phantom{risesabovesome} 

\vspace{-0.48in}
\begin{minipage}{.68\textwidth}
rises above some predetermined limit. The drawing to the right shows how to use an LM311 comparator as the basis for such a circuit.  If $V_+<V_-$, then pin 7 gets connected to pin 1 inside the chip\footnotemark, so that $V_{\rm out}=0$.  Otherwise, $V_{\rm out} \approx 5$~volts. 

\bigskip

Use your LM311 comparator to build a circuit with the following output: 

\smallskip
\begin{itemize}[nosep]
\item If $V_{\rm in} < 2.5$~volts, then $V_{\rm out}=5$~V.  (\textit{Output high when input low.})
\item If $V_{\rm in} > 2.5$~volts, then $V_{\rm out}=0$~V.  (\textit{Output low when input high.})
\end{itemize}
\smallskip
...
\hspace{\fill}\includegraphics{digital_electronics/basic_lm311.pdf}
\end{minipage}
