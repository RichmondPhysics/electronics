...
\item Identify a range of different capacitors in your kits.  What are the largest and smallest values that you have? How do you read the value of the capacitance on the small ones?  Use your DMM to measure some of their values, noting both the uncertainty in your DMM's measurement and how close the capacitors are to their stated values.
...
\item Make a circuit like the one above with a time constant of $\tau=10^{-4}~{\rm s} = 100~\mu {\rm s}$.   To verify the time constant with your oscilloscope, you will need to mess with the triggering for your scopes, so that the scope triggers only once as soon as you throw the switch, and doesn't trigger again.  You will need to change the trigger mode from ``auto'' to ``normal.''  (Look in your manuals.  What do these two modes mean?)
...
\begin{flushright}
\includegraphics{capacitors/delay.pdf}\hspace*{0.9in}
\end{flushright}

\item Modify the circuit slightly, replacing the switch with feedback from the output as shown below.   Draw three graphs of $V_+$, $V_-$, and $V_{\rm out}$, all versus time, showing how they are synchronized with each other. \label{part_square_generator}
 
\begin{flushright}
\includegraphics{capacitors/comparator_square_generator.pdf}\hspace*{0.9in}
\end{flushright}
