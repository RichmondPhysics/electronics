...
In this lab, we'll look closely at the behavior of an npn bipolar junction transistor under different operating conditions, simultaneously measuring the voltage and current at all three transistor terminals.  Rather than trying to scrounge up six multimeters for the job, we'll simulate the transistor's behavior in Multisim instead.

\textbf{BJT Operating Modes}
...
\begin{center}
\includegraphics{bjt_saturation/bjt_saturation1.pdf}
\index{color_page}
\end{center}
...
\begin{itemize}[nosep]
\item \textbf{Cut-off mode:} all currents $\approx 0$.  $V_{BE} \lesssim 0.7$~V, and $V_C > V_B$.%
\footnote{Authors differ on the exact definition of these modes.}  
\item \textbf{Active mode:} $V_C > V_B > V_E$  (e.g. a working amplifier).
\item \textbf{Saturation mode:} $V_B > V_C > V_E$.
\end{itemize}
Label each row of your data table to show which operating mode your transistor is in.

\item The \textit{bias} on a p-n junction is the applied voltage placed across it, which we categorize as follows:
\begin{itemize}[nosep]
\item A junction is ``zero-biased'' or ``unbiased'' when the applied voltage is zero.
\item A junction is ``reverse-biased'' when the n-type material is at a higher potential than the p-type material (i.e. no current flows).
\item A junction is ``forward-biased'' when the p-type material is at a higher potential than the n-type material (i.e. current flows for a forward bias of $\gtrsim 0.7$~V).
\item Occasionally, for clarity, we may say that a junction is ``sub-threshhold 
forward-biased'' to specify a forward bias of $\lesssim 0.7$~V.
\end{itemize}
For each of the three operating modes above, indicate the type of bias on both the emitter-base junction and the collector-base junction. 
(Remember, the transistor you are using is an npn BJT.)

\pagebreak[3]
\textbf{Bipolar Junction Transistors as Logical Inverters}
