...
\textbf{Primaries and Secondaries in Series and Parallel}
...
\begin{center}
\includegraphics{transformers/transformer_par_par.pdf}
\end{center}
...
\begin{center}
\includegraphics{transformers/transformer_par_par_reversed.pdf}
\end{center}
...
\begin{center}
\includegraphics{transformers/transformer_ser_ser_inverted.pdf}
\end{center}

\item What accounts for the slight discrepancy between the apparent turns ratios you measured in the last two parts?


\end{enumerate}
\bigskip
\textbf{Impedance Matching}

If a transformer steps the voltage UP, it has to step the current DOWN and vice-versa.  (If it stepped both of them up, then it would be stepping up the power, which would violate conservation of energy.)  The way the transformer ``steps up the current'' is by effectively having a lower output impedance than the original source.  Although this can't buy you power for free, you'll see in this next part that transformers can help you match impedances to use the power you have more efficiently.
...
\label{part_step_down_speaker}

\item In previous labs, you have found that the output impedance of your function generator is roughly $600\ohm$, and the input impedance of your speaker is roughly $8\ohm$.  
Based on the turns ratio that you used in part \ref{part_step_down_speaker}, what is the apparent load resistance ``seen'' by the source?  What is the apparent source resistance ``seen'' by the load?

\item What would be the ideal turns ratio for a transformer in order to transfer the maximum power from the function generator to the speaker, given the result you found in Problem \ref{prob_impedance_matching}?
...
\item Determine the optimal turns ratio for a transformer in order to optimally match the impedance of a load to the impedance of a source.
\end{itemize}
