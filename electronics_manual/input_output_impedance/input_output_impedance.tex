...
\item The short-circuit current measurement in the last part is okay for a relatively high impedance device like your function generator, but it is potentially dangerous for low impedance devices that can produce very large currents.  (Don't try short circuiting a car battery, for instance, or you could melt all your wires and cause the battery to explode!)  A safer method is to put a known load resistance $R_{LOAD}$ across the output terminals, and measure the voltage drop across $R_{LOAD}$.  By doing this, you have essentially made a voltage divider circuit using $R_{LOAD}$ and $R_{INTERNAL}$. Try this for your function generator.  What size resistor should you choose for $R_{LOAD}$?  How do your results compare with the previous part? \label{part_func_gen_impedance}
