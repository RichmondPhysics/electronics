...
%\item A low-pass filter uses a 22~nF capacitor and a 4.7~kΩ resistor.  What is the ratio $V_{\rm out}/V_{\rm in}$, in dB, at a frequency of 80~kHz?
...
\item Draw a ``phasor diagram'' for an $RL$ circuit.  (That is, on the complex plane, draw vectors representing $V_{\rm source}$, $V_R$, $V_L$, and $I$, at time $t=0$.)  Show how your drawing changes for $\omega \ll R/L$ 
