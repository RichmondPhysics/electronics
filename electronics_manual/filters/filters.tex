...
Because capacitors and inductors have frequency-dependent impedances, we can use them to build \textit{filters} that ``filter out'' some frequencies but not others.  

\medskip
\textbf{High-Pass and Low-Pass Filters}

\begin{enumerate}[wide]

\item Build the circuit below, connecting $V_{\rm in}$ to your function generator, and measure $V_{\rm in}$ and $V_{\rm out}$ for different frequencies.  (Use 10~Hz, 30~Hz, 100~Hz, 300Hz,  etc., up to 100~kHz.)  
Use Mathematica or Excel to make a \textit{Bode plot} for your circuit.  
A Bode plot is a graph of $\displaystyle \frac{|V_{\rm out}|}{|V_{\rm in}|}$ in decibels vs. the frequency $f$, with $f$ plotted on a log-scale horizontal axis.  Make a printout of your graph and tape it into your notebook.  
\label{part_low_pass_RC_bode}
\begin{center}
\includegraphics{filters/low_pass_filter_RC.pdf}
\end{center}

\item A \textit{low-pass} filter filters out high frequencies, allowing low frequencies to ``pass through.''  A \textit{high-pass} filter filters out low frequencies, allowing high frequencies to pass through.  Which kind of filter did you just build?  

\item Show on your graph the approximate frequency at which $V_{\rm out}/V_{\rm in}=-3$~dB, called the ``cutoff frequency,'' $f_\text{c}$.  Is your value of $f_\text{c}$ consistent with the expression you derived for $f_\text{c}$ in problem~\ref{prob_RCcircuit_AC}?  \label{part_low_pass_log_plot}

\item Measure the phase shift $\phi$ in degrees between $V_{\rm out}$ and $V_{\rm in}$ as a function of $f$, and make a rough plot of it.  You don't need many data points for this: say one or two at $f \ll f_\text{c}$, one or two at $f \gg f_\text{c}$, and several around  $f \approx f_\text{c}$. 
Be careful with your signs: if $V_{\rm out}$ \textit{lags} $V_{\rm in}$, the phase shift is \textit{negative}.  Check that your result is consistent with the expression you derived for $\phi$ in problem~\ref{prob_RCcircuit_AC}.

If you haven't already, check your graphs with your instructor now to make sure you're on the right track.


\item Design and build a \textit{high-pass} filter by switching the positions of the capacitor and the resistor in your previous circuit.  Change the value of the capacitor so that the cutoff frequency is 100~Hz.  Test your filter by recording $V_{\rm in}$ and $V_{\rm out}$ vs. $f$ and making a Bode plot as you did in part~\ref{part_low_pass_RC_bode}.  \label{part_high_pass_RC}

\medskip
\textbf{Filters Using Inductors}
...
\item The circuit below shows an easy way to add a DC offset (Also called a ``DC bias'') to an AC signal.  Use the principle of superposition to analyze this circuit, shorting out $V_{\rm in}$ and $V_{\rm DD}$.  In each case, what is $V_{\rm out}$ for both very high and very low frequencies?  Build it and test your predictions. \label{part_DC_bias}
\begin{center}
...
\item Design a band pass filter between 10~Hz and 1~MHz, using two capacitors and two resistors.  Choose resistor values so that the input impedance of the second stage doesn't affect the functioning of the first stage.  Also choose the resistor values and the order of the filters to avoid capacitors that are awkwardly sized, say either below 50~pF or above 100~nF.
...
\item What is the ratio of $V_{\rm out}/V_{\rm in}$, in decibels, of a high-pass filter at its cutoff frequency, $f_\text{c}$?
